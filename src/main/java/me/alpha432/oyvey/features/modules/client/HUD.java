...
public class HUD extends Module {
    ...
    public Setting<String> command = register(new Setting("Command", "ChickenWings.cc"));
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.waterMark.getValue().booleanValue()) {
            String string = this.command.getPlannedValue() + " v0.05";
    ...
    public void renderLag() {
        ...
        if (OyVey.serverManager.isServerNotResponding()) {
            String text = ChatFormatting.RED + "Server not responding (nice server/internet) " + MathUtil.round((float) OyVey.serverManager.serverRespondingTime() / 1000.0F, 1) + "s.";
