...
public class AutoDbear
        extends Module {
    private static AutoDbear INSTANCE = new AutoDbear();
    ...
    public static AutoDbear getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new AutoDbear();
        }
        return INSTANCE;
    }
    ...
    public void onRender3D(Render3DEvent event) {
        assert (AutoDbear.mc.renderViewEntity != null);
        Vec3i playerPos = new Vec3i(AutoDbear.mc.renderViewEntity.posX, AutoDbear.mc.renderViewEntity.posY, AutoDbear.mc.renderViewEntity.posZ);
        for (int x = playerPos.getX() - this.range.getValue(); x < playerPos.getX() + this.range.getValue(); ++x) {
            for (int z = playerPos.getZ() - this.range.getValue(); z < playerPos.getZ() + this.range.getValue(); ++z) {
                for (int y = playerPos.getY() + this.rangeY.getValue(); y > playerPos.getY() - this.rangeY.getValue(); --y) {
                    ...
                    if (!AutoDbear.mc.world.getBlockState(pos).getBlock().equals(Blocks.AIR) || !AutoDbear.mc.world.getBlockState(pos.add(0, 1, 0)).getBlock().equals(Blocks.AIR) || !AutoDbear.mc.world.getBlockState(pos.add(0, 2, 0)).getBlock().equals(Blocks.AIR) || pos.equals(new BlockPos(AutoDbear.mc.player.posX, AutoDbear.mc.player.posY, AutoDbear.mc.player.posZ)) && !this.renderOwn.getValue().booleanValue() || !BlockUtil.isPosInFov(pos).booleanValue() && this.fov.getValue().booleanValue())
                        continue;
                    if (AutoDbear.mc.world.getBlockState(pos.north()).getBlock() == Blocks.BEDROCK && AutoDbear.mc.world.getBlockState(pos.east()).getBlock() == Blocks.BEDROCK && AutoDbear.mc.world.getBlockState(pos.west()).getBlock() == Blocks.BEDROCK && AutoDbear.mc.world.getBlockState(pos.south()).getBlock() == Blocks.BEDROCK && AutoDbear.mc.world.getBlockState(pos.down()).getBlock() == Blocks.BEDROCK) {
                    ...
                    if (!BlockUtil.isBlockUnSafe(AutoDbear.mc.world.getBlockState(pos.down()).getBlock()) || !BlockUtil.isBlockUnSafe(AutoDbear.mc.world.getBlockState(pos.east()).getBlock()) || !BlockUtil.isBlockUnSafe(AutoDbear.mc.world.getBlockState(pos.west()).getBlock()) || !BlockUtil.isBlockUnSafe(AutoDbear.mc.world.getBlockState(pos.south()).getBlock()) || !BlockUtil.isBlockUnSafe(AutoDbear.mc.world.getBlockState(pos.north()).getBlock()))
                        continue;
