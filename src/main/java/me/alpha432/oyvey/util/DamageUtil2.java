...
public class DamageUtil2
    ...
    public static boolean isArmorLow(EntityPlayer player, int durability) {
        for (ItemStack piece : player.inventory.armorInventory) {
            ...
            if (DamageUtil2.getItemDamage(piece) >= durability) continue;
            return true;
        }
        return false;
    }
    ...
    public static float getDamageInPercent(ItemStack stack) {
        return (float) DamageUtil2.getItemDamage(stack) / (float)stack.getMaxDamage() * 100.0f;
    }

    public static int getRoundedDamage(ItemStack stack) {
        return (int) DamageUtil2.getDamageInPercent(stack);
    }
    ...
    public static boolean canBreakWeakness(EntityPlayer player) {
        int strengthAmp = 0;
        PotionEffect effect = DamageUtil2.mc.player.getActivePotionEffect(MobEffects.STRENGTH);
        if (effect != null) {
            strengthAmp = effect.getAmplifier();
        }
        return !DamageUtil2.mc.player.isPotionActive(MobEffects.WEAKNESS) || strengthAmp >= 1 || DamageUtil2.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword || DamageUtil2.mc.player.getHeldItemMainhand().getItem() instanceof ItemPickaxe || DamageUtil2.mc.player.getHeldItemMainhand().getItem() instanceof ItemAxe || DamageUtil2.mc.player.getHeldItemMainhand().getItem() instanceof ItemSpade;
    }
    ...
    public static float calculateDamage(double posX, double posY, double posZ, Entity entity) {
        ...
        if (entity instanceof EntityLivingBase) {
            finald = DamageUtil2.getBlastReduction((EntityLivingBase)entity, DamageUtil2.getDamageMultiplied(damage), new Explosion((World) DamageUtil2.mc.world, null, posX, posY, posZ, 6.0f, false, true));
        }
        return (float)finald;
    }
    ...
    public static float getDamageMultiplied(float damage) {
        int diff = DamageUtil2.mc.world.getDifficulty().getId();
        return damage * (diff == 0 ? 0.0f : (diff == 2 ? 1.0f : (diff == 1 ? 0.5f : 1.5f)));
    }

    public static float calculateDamage(Entity crystal, Entity entity) {
        return DamageUtil2.calculateDamage(crystal.posX, crystal.posY, crystal.posZ, entity);
    }

    public static float calculateDamage(BlockPos pos, Entity entity) {
        return DamageUtil2.calculateDamage((double)pos.getX() + 0.5, pos.getY() + 1, (double)pos.getZ() + 0.5, entity);
    }

    public static boolean canTakeDamage(boolean suicide) {
        return !DamageUtil2.mc.player.capabilities.isCreativeMode && !suicide;
    }
